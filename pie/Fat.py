...
class Fat(object):
	...
	def is_no_significant_meaning(self, cluster):
		return True if self.fat[cluster] == NO_SIGNIFICANT_MEANING else False
	...
	def get_cluster_chain(self, cluster):
		if self.is_eof(cluster) or self.is_no_significant_meaning(cluster):
			return [cluster]
